...
void SubsystemManager::startup()
{
    ...
    global::logger->debug << "Starting up subsystems." << std::endl;
    // iterate through the subsystems and start them up them (but only once)
    std::unordered_set<omi::ss::Subsystem*> started;
...
void SubsystemManager::shutdown()
{
    ...
    global::logger->debug << "Shutting down subsystems." << std::endl;
    // iterate through the subsystems and boot them (but only once)
    std::unordered_set<omi::ss::Subsystem*> shutdown;
    for(auto role : m_assigned)
    {
        for(omi::ss::Subsystem* subsystem : role.second)
        {
            // check if it's been shutdown already
            if(shutdown.find(subsystem) == shutdown.end())
            {
                subsystem->shutdown();
                shutdown.insert(subsystem);
            }
        }
    }
